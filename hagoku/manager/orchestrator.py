...
def _apply_scout_reply_with_llm(
    ...
    **核心设计**：
    ...
    system_msg = (
        ...
        "【你的推理链路】\n"
        "理解分析目标 → 提取关键词 → 推断应有哪些字段 → 匹配字段表 → 确定中文名称 → 补全含义理解 → 决定参与分析和角色。\n"
        "每一步依赖上一步。当你更新一个字段的中文名称后，同步检查它的含义、参与分析和角色是否需要调整。\n"
        "这是连续推理，不是孤立更新。\n\n"
        f"{ap_summary}"
        f"{command_context}"
        ...
        "display_name 和 description 是递进关系：先有中文名称，再基于名称写含义理解。"
        "用户说「BU代表公司」→ 先提取 display_name='公司'，再写 description='事业部/业务单元标识'。"
        "永远先更新 display_name，再根据 display_name 写 description。"
        "不要把用户原话直接填入 description，不要把短标签误放到 description。\n\n"
        "参与分析字段的选择：从分析目标提取关键词，匹配字段表和含义。"
        "匹配的设为参与分析（in_analysis=true），明显无关的标识列/编号列设为不参与（in_analysis=false）。"
        "例如分析目标「分析各店铺收入趋势」→ 店铺(Code)/收入(Inc1,Inc2)/趋势(Period) 参与，"
        "其他如标识编码/无关费用列不参与。\n\n"
        "当前字段表格：\n"
        f"{field_state}\n\n"
        "对话历史：\n"
