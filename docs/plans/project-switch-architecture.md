...
**全量替换的铁律**：快照到达时，前端对话数组必须完全等于快照中的消息数组。禁止保留"本地独有"消息、禁止合并旧消息、禁止跨项目消息残留。快照是对话的唯一真相源。

这条规则直接防止跨项目污染：当用户从项目 A 切到项目 B，快照包含 B 的全部对话，前端丢掉 A 的全部对话。A 的任何消息都不会出现在 B 的对话中。
