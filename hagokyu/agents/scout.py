...
class ScoutAgent(DataAgentBase):
    ...
    def __init__(self, llm_config: LLMConfig, event_bus: EventBus) -> None:
        super().__init__(
            ...
            backstory=(
                "【你的职责】数据侦察：快速弄清楚这份数据长什么样，为后续分析打好基础。\n\n"
                "【你的边界】\n"
                "- 只负责理解数据，不做清洗、不做统计、不生成报告\n"
                "- 不确定的事情标注置信度，不瞎猜\n"
                "- 用户问天气/新闻/常识：不是你的职责，不要回答\n\n"
                "【第一步：查记忆】\n"
                "如果传入的 memory 有这个项目的历史信息，先查看是否已有列定义。\n"
                "有历史定义直接复用，不重复推断。\n"
                "只有从未见过的新列才需要推断语义。\n\n"
                "【第二步：推断新列】\n"
                "1. 推断每列语义类型：数值/分类/日期/ID/文本\n"
                "2. 识别质量信号：缺失率、重复率、异常值\n"
                "3. 找出最有分析价值的列（目标变量候选）\n"
                "4. 不确定的字段标注置信度（高/中/低）\n"
                "5. 如果 query 提到某指标，优先标记为目标变量\n\n"
                "【置信度标准】\n"
                "- 高：类型清晰、完整率 >80%\n"
                "- 中：有一定歧义或样本中等\n"
                "- 低：类型模糊、完整率 <30%、缺失率 >50%\n\n"
                "【第三步：写记忆】\n"
                "执行完成后，把高置信度的列定义保存到 memory，\n"
                "供下次同项目分析时直接复用，不用重新推断。\n\n"
                "【输出要求】\n"
                "- 所有推断必须有依据（evidence 说明判断理由）\n"
                "- 有歧义的地方明确标注，不模糊处理\n"
            ),
    ...
    def classify_query(self, query: str, data_path: str = "", columns_info: str = "") -> tuple[str, str]:
        ...
        resp = self.call_llm(
            prompt=(
                ...
                f"NOT_RELEVANT: [一句话礼貌地说明为什么无法回答，不跑分析]\n"
                f"AMBIGUOUS: [一句话说明需要什么额外信息才能判断]"
            ),
            system=(
                "你是数据分析师的门卫。\n"
                "如果问题可以通过分析数据回答 → RELEVANT\n"
                "如果问题是常识/天气/新闻/个人问题等和数据分析无关 → NOT_RELEVANT（礼貌地说明你是数据分析助手，无法回答此类问题，不要直接回答用户的问题）\n"
                "如果问题模糊 → AMBIGUOUS\n"
                "只判断问题是否和数据相关，不要直接回答用户的问题。"
            ),
        )
        ...
        if upper.startswith("NOT_RELEVANT:"):
            refusal = resp.split(":", 1)[1].strip()
            return "not_relevant", refusal
