...
**绝对禁止未经测试就对 LLM 模型做任何功能性猜测。** 包括但不限于：
- "这个模型不支持 X 功能"
- "MiniMax 的 instructor 模式可能有问题"
- "模型响应慢导致超时"
- 任何以模型能力为借口、回避排查代码问题的行为

**必须先跑测试、查 dump、读日志，用证据定位问题。** 测试通过但功能异常 → 问题在代码，不在模型。
模型连通性可以单独 curl 验证，但不允许用"可能模型不行"作为跳过代码排查的理由。
