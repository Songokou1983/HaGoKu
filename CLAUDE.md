...
## ⛔ 最高优先级：遇到任何问题先读数据，不问用户

session.json = 前端对话的镜像。llm_dumps = LLM 输入输出的快照。hagoku.log = 所有事件。
禁止问"你看到了什么"——数据已经告诉你了。

## ⛔ 架构铁律：对话就是消息列表

不分文字、表格、卡片。产出一条加一条。一条路。
不要给 LLM 产出分类。不要另建卡片系统。所有成功软件都这么做。
...
**核心竞争力：通道 × 提示词 = 多维度分析。** 不改代码，只换 prompt，同一个引擎适配不同领域（股市/电商/医疗）。预设系统在 `hagoku/agents/presets/`，用户选择预设后 `_load_prompt()` 读 `~/.hagoku/active_preset`。新建领域 = 写一段 prompt，零代码。详见 `PROJECT.md` 和 `docs/PRESETS.md`。**任何改动不得破坏通道通用性——通道里不能出现领域特定的硬编码。**
...
| **12** | **无日志不猜测** | **没有日志证据时禁止说"可能是"。停下来补日志系统，拿到证据再开口。** |
...
| **H** | 同一问题反复出现 → 加一行/加一个if | **补丁思维。** 先问：是不是架构设计就有问题？数据有几个存储？有没有单一真相源？改结构比加代码有效。症状在此处 ≠ 根因在此处。 |
...
| 用户 | 代码-AI |
|------|--------|
| 点击应用图标启动软件 | 重启 API、重启前端 dev server |
| 在应用内操作（点项目、输入文本、开始分析） | 运行命令（pytest、tsc、grep 验证） |
| 观察界面行为、反馈问题 | 读日志、读 dump、读 session 定位根因 |
| 决策方案方向、确认改动 | 提出方案、等确认后动手 |
| 不负责让更新的代码生效 | 确保每次改动后服务运行最新代码 |
