# 减法的哲学 · The Philosophy of Subtraction
...
这个阶段诞生的铁律、刹车、触发词——不是"学到的知识"，是"我终于能把脑子里那种不对的感觉说清楚了"。
...
同一个 bug，同样的症状，AI 给出了两个补丁。第 5 天的你会说"好，加进去"。今天的你看到 15 行新增，沉默了。看到"添加诊断日志"，警觉了。然后你做的不是 merge，是**要求重新定位根因**。
...
同一天。早晨 30 commits，两次撤回，删 87 行——**清理。** 晚间 4 commits，零撤回，加 243 行——**建设。**

清理是建设的前提。不是先后顺序——是因果关系。不清理就建设不了。

### 第八阶段：哲学验证（7/10）

"通道不是一个技术方案——是一套可复现的原则。"

预设系统（同通道换提示词适配三领域）、删 JSON mode（代码不绑模型能力）、Doctor 操作手册（代码不硬编码规则）、Doctor 闭环（LLM 输出 `[fix:xxx]`，代码自动执行）。四个里程碑在同一天落地，各自从不同维度验证同一个原则。
...
**通道违轨撤回**（24 次）：代码替 LLM 做了判断。修复方式永远是删代码。

**协议调试撤回**（4 次）：技术问题试错（WebSocket keepalive）。同样慎重，但结论不同——违轨撤回后删代码，协议撤回归因是换方案。
...
LLM 流式已经送达前端。禁止从 context 取 LLM 文本再发一次。删掉搬运代码用户看到的没变 → 搬运。变了 → 原有通道断裂。
...
用户最后说了什么？LLM 回了什么？工具调用了吗？**第一天就让 AI 帮你配好 dump，你只需要 `cat` 一个文件。**
...
同一功能区域反复出问题 → 不修，重建架构。让 AI 维护 `CHANGES.md`。同一模块频繁出现 → 🔴 → 喊停。

### 警惕"加诊断日志"

AI 找不到根因时的默认反应。问 AI："在不加一行日志的前提下，这个 bug 的可能根因有哪些？"——强迫推理模式，不进补丁模式。
