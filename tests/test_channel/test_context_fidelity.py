...
def _standard_scenario() -> tuple[Session, dict[str, Any]]:
    """标准场景：Scout 2 轮纠正，用于基线回归测试。"""
    session = Session(analysis_goal=GOAL)
    session.add("user", "Inc1 是净利润不是总收入")
    session.add("assistant", "已更新 Inc1 描述")
    session.add("user", "BU 不参与分析")
    session.add("assistant", "已排除 BU 字段")
    return session, _ctx(session)
...
class TestCorrectionContinuity:
    ...
    def test_第一轮纠正在第三轮messages中仍可见(self):
        ...
        session.add("user", c1)
        session.add("assistant", "已更新 Inc1")
        session.add("user", c2)
        session.add("assistant", "已排除 BU")

        msgs = session.to_llm_messages(user_input="继续")
        text = _text(msgs)
    ...
    def test_五轮纠正全部保留(self):
        ...
        msgs = session.to_llm_messages(user_input="确认")
        text = _text(msgs)
...
class TestBaselineRegression:
    ...
    def test_messages条数不低于基线(self):
        session, ctx = _standard_scenario()
        msgs = session.to_llm_messages(user_input="确认")
    ...
    def test_最后一条是user且含当前输入(self):
        session, ctx = _standard_scenario()
        msgs = session.to_llm_messages(user_input="这是最新用户输入_独特标记")
    ...
    def test_重构前后messages条数不减少(self):
        ...
        def build(n: int) -> list[dict]:
            ...
            for i in range(1, n + 1):
                session.add("user", f"纠正_{i}_{i*3333}")
                session.add("assistant", f"已处理_{i}")
            return session.to_llm_messages(user_input="继续")
...
class TestCrossStageTransfer:
    ...
    def test_tool_exchange以标准协议出现在messages_history(self):
        ...
        session = Session(analysis_goal=GOAL)
        oai_calls = [{"id": "tc-001", "type": "function", "function": {"name": "update_field_understanding", "arguments": '{"column_name":"Inc1","display_name":"净利润"}'}}]
        results = [{"content": '{"status":"ok"}', "tool_call_id": "tc-001"}]
        session.add_tool_call("更新字段理解", oai_calls, results)
    ...
    def test_scout纠正在cleaner阶段messages_history中可见(self):
        ...
        session.add("user", scout_msg)
        session.add("assistant", "字段理解完成")
        session.add("system", "[进入 cleaner 阶段]")
    ...
    def test_tool_error保留在messages中(self):
        ...
        session = Session(analysis_goal=GOAL)
        oai_calls = [{"id": "tc-err-001", "type": "function", "function": {"name": "update_field_understanding", "arguments": '{"column_name":"Inc1"}'}}]
        results = [{"content": "字段不存在：Inc1 未在 column_semantics 中注册", "tool_call_id": "tc-err-001"}]
        session.add_tool_call("尝试更新", oai_calls, results)

        msgs = session.to_llm_messages(user_input="修正")
        tool_msgs = [m for m in msgs if m.get("role") == "tool"]
...
class TestContextOrder:
    ...
    def test_动态上下文排在静态指令前面(self):
        ...
        msgs = session.to_llm_messages(
            system_extra=agent_instructions, user_input="开始",
        )
...
class TestFailureRoundPreservation:
    ...
    def test_失败轮次用户原话保留在后续messages(self):
        ...
        session.add("user", failure_input)
        session.add("assistant", "[未理解] LLM 未产生有效工具调用")
        session.add("user", "我的意思是：StoreID 是标识符，Period 是时间，Inc1 是目标")

        msgs = session.to_llm_messages(user_input="请重新理解")
        text = _text(msgs)
    ...
    def test_多阶段失败轮次均保留(self):
        ...
        session.add("user", scout_fail)
        session.add("assistant", "[未理解]")
        session.add("assistant", "字段完成")
        session.add("system", "[进入 cleaner 阶段]")
        session.add("user", cleaner_fail)
        session.add("assistant", "[未理解]")
        session.add("assistant", "清洗完成")
        session.add("system", "[进入 analyst 阶段]")

        msgs = session.to_llm_messages(user_input="开始分析")
        text = _text(msgs)
