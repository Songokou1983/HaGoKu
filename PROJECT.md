...
文档里曾并列「**话术动态**（暂停点可由 LLM 依结果生成）」与「**反冒充对话**（禁止写死大段 Agent 台词）」。二者冲突时的 **P0 裁决**：
...
- **价值优先于形式**：交付物要有信息价值——结论站得住、统计要素齐、对用户问题有回应；**不比谁先生成「长得像报告」的 HTML**。
- **真诚**：专业底线未满足时，**不**把分析包装成已验收的正式成品；与其误导，不如明确「此处尚未达标」。
- **有问题就沟通**：护栏触发（尤其强制级）不是流程的静默终点；产品与 **「人机互动理念」**（上文）一致——**向用户说明缘由、可读的影响、以及建议的下一步**（改问法、补数据、换方法、或在充分知情前提下再议），让协作闭环有意义。
- **分级处置**：**强制级**＝未过则**不**走正式 Reporter 双轨成品（避免假阳性交付），并**落盘可读说明**（默认 `runs/{run_id}/output/GUARDRAILS_BLOCKED.md`，并可通过事件流追溯）；**警告 / 提示级**＝仍可将报告产出，但须在呈现层**显著可见**风险与建议，而非藏在日志里。

以下为规则分级与工程锚点；**满足各级规则是「有价值」的必要条件，不是替代与用户对话。**
...
| [DEVELOPMENT_PROMPT.md](DEVELOPMENT_PROMPT.md) | **路线图跟踪（四阶段）** + 单轮任务传递模板；审查约定见该文件 | 派活人、协作者、审查人 |
