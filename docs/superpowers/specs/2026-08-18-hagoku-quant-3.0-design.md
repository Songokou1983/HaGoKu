...
**核心约束**：「最大化的利用现有的框架」。现状是「项目名 + 描述」，3.0 **只加一个东西**：场景下拉。数据源选择在分析页面（见 §4.6），不在这里。

#### 4.4.1 现状（参考，hagoku_web/src/panels/ProjectPanel.tsx:340）
...
- 项目创建时选定场景，写入 `project.json`
- 默认场景：`"general"`（通用商业分析）
- 场景可后续修改（在 ProjectPanel 项目列表里，点场景标签可改）
- 全局 `~/.hagoku/active_preset` 保留作为新建项目时的默认值
- 全局 active_preset = 用户上次创建项目时选定的场景
...
**现状**（hagoku_web/src/panels/AnalyzePanel.tsx）：进入分析页面后用 `useFileUpload` hook 上传 CSV，路径写入 `currentDataPath`。

**3.0 改造**：在现有「上传 CSV」旁边，加一个「从量化数据集选取」。
...
**「从量化数据集选取」子流程**：
1. 弹下拉，列出 `~/.hagoku/datasets/<id>/meta.json` 中的所有数据集
2. 用户选一个
3. 后端从对应目录读 `data.parquet`
4. 复制进当前项目状态（副本模式，与现有 CSV 上传一致）
5. `currentDataPath` 更新为「数据集路径」标识

**关键不变**：分析页面其余部分（对话 / 工具调用 / 报告生成）**完全不动**。LLM 不知道数据来自 CSV 还是数据集库——对它来说都是「当前项目 DataFrame」。

**LLM 拉新数据**：用户在分析中说「拉 BTC 90 天数据」，LLM 调 `fetch_market_data`：
- 数据落到 `~/.hagoku/datasets/`（独立文件）
- 同时复制进项目状态
- 用户在量化数据集 tab 看到新条目 + 分析页可用新数据
