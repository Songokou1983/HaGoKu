...
> ⚠️ **归档说明**（2026-07-24）：本文档撰写于 Phase D（4 agent 合 1，2026-06-11）之前。引用的旧 gate 机制函数（`_pause_and_wait`、`_is_scout_aligned` 等）已删除。本文档保留作为契约设计参考，接口定义已不适用。
...
**C4 相关用例**：`test_is_scout_aligned_*`、`test_interaction_revision_in_scout_payload`（及 Scout 多轮行为与 `_pause_and_wait` 编排变更的回归，变更时必跑）。**C5**：`test_c5_cleaner_analyst_proceed_helpers`。

新增禁止类规则时：**先在本表增加一行，再写/改测试**，避免「只改 PROJECT」。

## 4. 已知缺口（非本契约能单独解决）

- **除 Scout 字段表外的「全自由多轮」**：Scout 已具备**确定性**多轮对齐子循环；**Cleaner / Analyst** 已具备同阶段多轮 + **C5** 显式放行（见 §1 表）。**跨阶段闸门**等仍见 [INTERACTION_MULTITURN_PLAN.md](INTERACTION_MULTITURN_PLAN.md) §2.2 / §4-B。更宽的状态图（对比 `PROJECT.md` LangGraph 对照）**不**由本契约假装已完备。
- **「成长」的可视叙事**：当前以记忆/知识/看板为主；若需 UI 级「履历时间线」，另立规格与测试。

**实施计划（多轮对齐）**：[INTERACTION_MULTITURN_PLAN.md](INTERACTION_MULTITURN_PLAN.md)。**C4（Scout）**、**C5（Cleaner / Analyst）**已写入 §1 表；跨阶段闸门等随方案 §4-B 与 `DEVELOPMENT_PROMPT` **2.8.3** 继续补测试与条文。
