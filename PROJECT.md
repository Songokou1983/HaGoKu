...
**为什么是本地模型？** 商业数据天然敏感。HaGoKu 的设计优先保障数据不出本机——用户在本地运行具备 tool calling 能力的模型，即可获得专业分析能力。同样兼容云端 API，用户可根据保密需求自行选择。

---

## 演进方向

> **Phase A-D 已完成（2026-06-11）**：4 agent 合 1 DataAnalystAgent + 27 工具全集 + Meta v2 四组件。[详见](docs/plans/2026-06-11-collapse-to-single-agent-brief.md)。
>
> **v0.9（2026-06-25）**：删除代码替 LLM 做阶段判断的所有逻辑。LLM 从对话历史自主判断当前阶段。状态持久化（重启恢复会话）。输入框永远可见 + 停止按钮。报告循环（报告生成后对话继续）。16 工具（含 generate_report）。[通道参考](docs/CHANNEL.md)。
...
HaGoKu 的架构产生了一个独特能力：**不改一行代码，仅切换提示词，就能将同一个分析引擎适配到完全不同的领域。**

```
           ┌──────────┐
           │  通道     │  ← 代码：纯透传 + 工具执行，不包含任何领域知识
           └─────┬────┘
                 │
    ┌────────────┼────────────┐
    │            │            │
┌───┴───┐  ┌────┴────┐  ┌───┴───┐
│通用商业│  │股市技术  │  │电商运营│  ← 提示词预设：注入领域术语和分析框架
└───────┘  └─────────┘  └───────┘
```

**为什么这是竞争力：**
- 传统方案：每个领域需要独立的 agent + 独立的工具集 + 独立的 prompt 工程
- HaGoKu 方案：同一个 DataAnalystAgent + 同一套 15 工具 + 不同的 prompt.md 预设 = 不同领域的分析能力
- 用户无需理解 LLM 或写代码——在「分析能力」面板一键切换，后续所有分析自动按该领域方向执行
- 新建领域零代码：写一段 prompt + 起个名字，立即可用

**预设管理**：`hagoku/agents/presets/` 目录，`presets.json` 元数据 + `.md` 提示词文件。默认「通用商业分析」不可删除，作为稳定基座。详见 [Prompt Lab 使用说明](docs/PRESETS.md)。
...
**通道不是"完全被动"。** HaGoKu 是数据分析产品，不是自由探索的 IDE。数据分析有固定骨架：必须先加载数据才能画像，必须先画像才能理解字段，必须先理解字段才能分析。代码负责确保这个骨架走完（**流程保障**），LLM 负责每一步里的具体内容（**语义决策**）。

| | 流程保障（代码的事） | 语义决策（LLM 的事） |
|---|---|---|
| 字段理解 | 确保"加载数据 → 画像 → 调 LLM → 展示给用户确认"这个序列发生 | 字段叫什么、是什么含义、该不该参与分析 |
| 清洗评估 | 确保"LLM 看到数据质量 → 给出评估 → 用户确认"这个序列发生 | 哪些列需要清洗、用什么策略 |
| 统计分析 | 确保"LLM 看到字段 + 目标 → 跑计算 → 产出发现"这个序列发生 | 选什么检验方法、怎么解读结果 |
| 阶段切换 | 不适用（v0.9 删除）—— LLM 从对话历史自主判断当前阶段，代码不做阶段推断 |

**检验标准**：代码决定了"做什么事"还是"事情按什么顺序发生"？前者越界，后者是流程保障。
...
**通道的首选机制**：function calling（tools）。代码定义工具签名，LLM 主动调用。代码仅机械执行 `msg.tool_calls` 的结果。
...
### 总纲：代码可以做一切辅助，只要不做决策、不碰 LLM 输出

> **代码能做什么？** 透传、执行工具、记录日志、dump 诊断、格式校验、统计计算、可视化渲染、守门检查。一切不替 LLM 做选择、不修改用户/LLM 信息的纯机械动作。
>
> **代码不能做什么？** 构造展示数据（field_review/cleaning_review/analysis_fields_summary）、翻译字段名、调用额外的 LLM 改写内容、判断用户意图、决定阶段切换。任何"把信息加工后再展示"的动作都是越界。
>
> **检验标准**：删除这段代码后，用户看到的内容是否变化？变 → 代码越界了。不变 → 代码是通道。
...
| 分析方法 | LLM 从训练数据（知识库）自行选择，代码提供执行工具（如 `run_statistical_test`） | 代码用 if-else 根据关键词选择分析方法 |
| 用户反馈处理 | 代码提供 `set_columns` function calling 工具，LLM 通过 tool_calls 主动选择更新哪些字段，代码机械写入 context | 代码用正则 `col=desc` 格式解析用户输入并自行更新字段 |
...
**新增工具只需在 `agent_tool_defs.py` 加一个 `Tool(...)` 注册**，指定 `phase_tag=["理解字段","评估清洗"]` 标注典型关注点。当前 15 个工具全集对 LLM 可见。
...
**当前工具清单**（16 个）详见 `docs/TOOL_DESIGN.md`。分五类：读数据（4）、跑计算（7）、写状态（1）、流程控制（3）、画图（1）+ 报告生成（1）。
...
| **例子** | ask_user 有几种格式、报告结论必须含四层 | 工具怎么选、字段怎么判断、离群值要不要洗 |
...
| **系统控制接口** | LLM 必须知道才能调的系统机制 | ask_user 格式、submit 工具时机、generate_report 用法 |
...
- **自然语言对话**：单 chat 贯穿全程，LLM 自主推进或暂停
...
HaGoKu 用全量对话历史驱动 LLM。每条分析过程的记录（LLM 的判断、用户的纠正、工具输出的统计结果）都保留在对话中，`build_messages()` 不做筛选。五阶段分析的上下文随对话线性增长。

### 上下文里装了什么

| 类别 | 示例 | 保留原因 |
|------|------|---------|
| 分析过程 | LLM 的判断文本、用户确认/纠正 | 分析过程本身 |
| 统计结果 | 相关系数、回归系数、p 值、效应量 | 分析结论的数字证据，LLM 写报告时需要逐条引用 |
| 计算原料 | `describe()` 输出、字段画像 | 已被 LLM 消化为分析过程文本，但 `build_messages()` 不做内容分类——去留由架构决定，不由代码判断 |

### 模型要求

HaGoKu 建议使用上下文窗口 **≥ 128K tokens** 的模型。当前主流本地模型（qwen2.5、llama3.1、deepseek-r1）和云端模型均满足。2 列门店数据集的部分分析实测 ~48K tokens，典型商业数据分析在 128K 窗口内有余量。

### 如果上下文爆了

上下文超过模型窗口上限时，LLM 会丢失早期对话。这不是 bug——是物理限制。HaGoKu 不做自动压缩（代码无权替 LLM 判断什么可以丢弃），但有两条路：

| 路径 | 说明 |
|------|------|
| **换更大的模型** | 128K → 200K+（如 Claude）或 1M（如 Gemini） |
| **给 LLM 管理工具** | 为 LLM 提供 `manage_context` 工具，由 LLM 自己决定何时将已完成阶段的对话压缩到磁盘。代码只执行 LLM 的决策 |

路径 2 尚未实现。当前设计假设 128K 窗口足够覆盖典型分析场景。如果用户遇到上下文溢出的实际案例，应优先验证是否为非预期的上下文膨胀（如重复消息、冗余数据）而非直接引入压缩机制。
...
| `docs/CHANNEL.md` | 通道开发指南（反模式经验 + 正确实现参考） | 开发者 + AI 助手 |
...
## 通道审计（2026-06-16 / 更新 2026-06-25 v0.9）
...
v0.9 前代码做了三件事：扫描 LLM 工具调用 → 硬编码表查"set_columns=评估清洗" → 往 prompt 塞"你当前在 XX 阶段"。这是铁律 1 禁止的。

v0.9 删除：`infer_current_focus()`、`phase_hint` 注入、prompt 的"当前阶段会标注"。LLM 从完整对话历史自主判断阶段，前端不显示阶段标签。

### prompt 演进

| 版本 | 行数 | 特点 |
|---|---|---|
| 旧 | 87 | 强制四阶段流水线、"只确认完成"沉默指令 |
| Phase D | 10 | 自由对话、工具全开 |
| v0.9 | 10 | 四阶段流程 + ask_user 确认点，代码不猜阶段 |
