...
**新增工具只需在 `agent_tool_defs.py` 加一个 `Tool(...)` 注册**。当前 ~12 个纯 I/O 工具对 LLM 可见。工具即 I/O——不携带流程信号。
...
| **例子** | 报告结论必须含四层 | 工具怎么选、字段怎么判断、离群值要不要洗 |
...
| **系统控制接口** | LLM 必须知道才能调的系统机制 | generate_report 用法 |
...
- **自然停顿**：LLM 输出文本后自然停止，用户回复后继续。`while tc_list` 自动续轮，对标 Claude Code。
...
| v0.9 | 10 | 自然停顿 + `while tc_list`，对标 Claude Code |
