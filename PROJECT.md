...
对应的设计实现：
- Manager 三模式（balanced / rule / ai）各有明确场景，按需选择
...
HaGoKu 不只是冷静地报告结果，而是理解用户的心态：
- 有显著发现时 → 肯定用户（"找到了 X 项显著发现，数据在说话"）
- 没有显著发现时 → 安慰用户（"未达显著不意味着分析失败，它排除了可能性"）
- 数据质量差时 → 给予安全感（"数据质量一般，我会谨慎处理，结论会标注限制"）
- 数据质量好时 → 给予信心（"数据质量不错，可以进行深入分析"）
- 交互限制 → 透明告知（"本轮已达上限，报告已保存"）

对应的设计实现：
- Scout：加载数据后提示"收到数据"，质量好/一般分别鼓励
- Cleaner：清洗完成时根据 bias_risk 给出对应鼓励（"数据干净，可以放心分析"）
- Analyst：分析完成后鼓励（"找到了 X 项显著发现"或"这也是有价值的结论"）
- Reporter：headline 生成时，非显著结果正面化（"数据不足以得出强结论"而非"无显著差异"）
- Guardrails：违规消息从"统计学家视角"翻译为"用户视角"（"VIF 超标"→"某些列信息太相似"）
- REPL loop：blocked 类型redirect而非拒绝，unknown 类型给予引导而非静默忽略
...
4. **模式可调** — 三种模式，按场景选择
   ```python
   MANAGER_MODES = {
       "balanced": {"rule_match": "llm_adjust"},  # 默认：规则匹配后由 AI 微调
       "rule":     {"rule_match": "direct"},       # 纯规则：规则匹配后直接执行
       "ai":       {"rule_match": "llm_generate"}, # AI 优先：无规则匹配时由 AI 生成
   }
   ```
   - **balanced**：适合大多数场景，规则兜底 + AI 优化
   - **rule**：适合快速、低资源消耗、对 AI 不信任的场景
   - **ai**：适合复杂/模糊问题，愿意等待 AI 推理的场景
