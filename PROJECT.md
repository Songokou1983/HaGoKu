...
**为什么是本地模型？** 商业数据天然敏感。HaGoKu 的设计前提是数据不出本机——用户在本地运行 35B+ 级别模型（如 Qwen-2.5-32B），获得专业分析能力的同时保障数据保密性。这不是对模型能力的妥协：35B 模型的 tool calling 和推理能力足以支撑完整的统计分析流程。
