...
| 阶段切换 | 机械执行 LLM 的 `route_to` 调用 | 什么时候该切换、切到哪里 |
...
**当前工具清单**（15 个）详见 `docs/TOOL_DESIGN.md`。分五类：读数据（4）、跑计算（5）、写状态（1）、流程控制（4）、画图（1）。
...
**唯一 DataAnalystAgent**（`hagoku/agents/agent.py`）。按 4 关注点工作（理解字段/评估清洗/跑统计/写报告），通过 `route_to` 自主切换。统一 prompt（`hagoku/agents/prompt.md`）。15 工具全集可见（详见 `docs/TOOL_DESIGN.md`）。
