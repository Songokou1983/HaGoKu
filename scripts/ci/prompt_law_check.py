...
_VERDICT_PATTERNS: list[tuple[str, str]] = [
    ...
    (r"禁止.{0,30}(?:调用|使用|输出|说|做|探查|探索)", "刹车G: 用禁止堵mismatch应改为对齐工具或字段"),
    (r"不要.{0,30}(?:调用|使用|输出|说|做|探查|探索)", "刹车G: 用不要堵mismatch应改为对齐工具或字段"),
    (r"不准.{0,30}(?:调用|使用|输出|说|做|探查|探索)", "刹车G: 用不准堵mismatch应改为对齐工具或字段"),
]
