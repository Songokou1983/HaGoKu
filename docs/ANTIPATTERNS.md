...
## 反模式 6：补丁循环 — 修一个问题引入一个新问题（2026-06-25 全天教训）

**触发**：遇到 bug → 加代码堵 → 堵出新 bug → 加更多代码 → 回到原点。一天内 9 个 commits，最终撤回 2 个。

**为什么错**：每个补丁单独看都合理，但补丁之间相互作用，且补丁从未解决根因。根因是 prompt 没写清楚工作要求，但代码在猜 LLM 的状态、替 LLM 做判断，猜错了就加 new 判断覆盖判断，越加越复杂。

**正确的诊断流程**：
1. **先看 dump** — 几十个 dump 文件记录了 LLM 每轮的输入和输出，不用猜
2. **区分代码 bug 和 prompt 缺失** — LLM 不调 ask_user 是 prompt 问题，不是代码问题。加代码替 LLM 判断"什么时候该停"永远不会对
3. **回测改动** — 改了代码之后对比新旧 dump，验证改了的行为真的变了、不该改的没变

**今天的具体教训**：

| 补丁 | 修了什么 | 引入什么 |
|------|---------|---------|
| 删内部循环改对话层 | LLM 每轮都停 | 对话层猜停点 → 清洗阶段卡住 |
| had_tools 条件 | 不卡干活宣言 | 代码又在猜"LLM 是不是在说话" |
| 撤回恢复原架构 | 不猜了 | 循环还在，LLM 自主权限大 |

**最后保留下来的正确改动**（5 个）：
- respond 改 run_in_executor（纯通道）
- 删阶段概念（代码不猜阶段）
- ask_user break（执行 LLM 决策）
- prompt 明确停点（LLM 主导）
- generate_report 工具（通道）

**撤回了 2 个**：对话循环、had_tools — 都是代码替 LLM 做判断。

**核心理念**：代码做的事 = LLM 自己不能做的（纯运算、IO）。LLM 能做的一律不替。通道补全（加工具、修信号）→ 对的。代码猜 LLM 意图 → 错的。
